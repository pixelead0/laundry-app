...
export const api = {
    async fetchMachines(): Promise<Machine[]> {
        const res = await fetch(`${getBaseUrl()}/machines`);
        if (!res.ok) throw new Error('Failed to fetch machines');
        return res.json();
    },

    async fetchTurns(): Promise<Turn[]> {
        const res = await fetch(`${getBaseUrl()}/turns`);
    ...
    async assignTurn(machineId: number, turnId?: number): Promise<void> {
        const url = `${getBaseUrl()}/machines/${machineId}/assign?${turnId ? `turn_id=${turnId}` : ''}`;
        const res = await fetch(url, { method: 'POST' });
        if (!res.ok) throw new Error('Failed to assign machine');
    },

    async completeMachine(id: number): Promise<void> {
        const res = await fetch(`${getBaseUrl()}/machines/${id}/complete`, {
            method: 'POST',
        });
    ...
    async setMaintenance(id: number): Promise<void> {
        const res = await fetch(`${getBaseUrl()}/machines/${id}/maintenance`, {
            method: 'POST',
        });
    ...
    async recoverMachine(id: number): Promise<void> {
        const res = await fetch(`${getBaseUrl()}/machines/${id}/recover`, {
            method: 'POST',
        });
    ...
    async cancelAssignment(id: number): Promise<void> {
        const res = await fetch(`${getBaseUrl()}/machines/${id}/cancel`, {
            method: 'POST',
        });
    ...
    async updateMachine(id: number, payload: any): Promise<void> {
        const res = await fetch(`${getBaseUrl()}/machines/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload),
        });
